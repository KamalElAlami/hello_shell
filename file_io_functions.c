...
int read_hist(info_t *info)
{
	...
	for (i = 0; i < fsize; i++)
		...
		{
			buf[i] = 0;
			make_hist_list(info, buf + last, linecount++);
			last = i + 1;
		}
	if (last != i)
		make_hist_list(info, buf + last, linecount++);
	...
	while (info->histcount-- >= HIST_MAX)
		_delete_noode_from_index(&(info->history), 0);
	re_number_hist(info);
	return (info->histcount);
}

/**
 * make_hist_list - adds entry to a history linked list
...
int make_hist_list(info_t *info, char *buf, int linecount)
{
	...
	if (info->history)
		node = info->history;
	_add_noode_end(&node, buf, linecount);
